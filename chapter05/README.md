...
exit
#Una vez montado en archivo en la ruta que determinamos se aplica el manifiesto multi-container-with-hostpath.yaml visualizamos la creacion de los contenedores con el comando kubectl get pods una vez esten en estado running ejecutamos el comando kubectl exec multi-container-with-hostpath -c nginx-container -- cat /foo/hello-world.txt e inmediatamente deberia imprimir "Hello Word"
```